# Start Hier - Logs

Gebruik dit bestand als vaste ingang voor voortgang en incidenten.

## Belangrijkste logbestanden

- `LOGBOEK.md`  
  Dagelijkse voortgang + incidenten + volgende stappen.

- `RUNBOOK_MAPS_LIVEGANG.md`  
  Operationeel stappenplan voor livegang van MAPS 1 en MAPS 2.

- `LOG_TEMPLATE.md`  
  Template voor nieuwe logsessies.

- `MAPS_TRILOGIE_OVERZICHT.md`  
  Inhoudelijke projectlijn voor Deel 1, 2 en 3.

## Laatste bekende focus

- Deel 1 flow en privilege wiel (13 assen + radar) staan goed.
- Blijvende aandacht: stabiele online save naar Supabase bij overgang naar reisverslag.
- Trilogie-scheiding bewaken: Deel 3-opdrachten alleen in Deel 3, niet in Deel 1.

## Werkwijze bij hervatten

1. Open `LOGBOEK.md` en lees de laatste sectie.
2. Open `RUNBOOK_MAPS_LIVEGANG.md` en volg de preflight/deployvolgorde.
3. Controleer huidige status in preview of productie.
4. Werk bij in `LOGBOEK.md` met datum, wijziging, resultaat, URL's en volgende stap.
