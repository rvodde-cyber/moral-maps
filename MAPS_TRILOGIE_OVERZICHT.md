# MAPS Trilogie - Overzicht Deel 1, 2 en 3

Werkversie gebaseerd op jouw huidige richting: zoveel mogelijk bestaande opdrachten behouden, met duidelijke plek in de reis.

## Deeloverzicht

| Deel | Reisfase | Kernbeweging | Hoofddoel | Beste plek |
|---|---|---|---|---|
| Deel 1 | Begin | Zelf leren kennen | Waarden en startpositie zichtbaar maken | Vertrekpunt |
| Deel 2 | Onderweg | Botsing en duiding | Kiezen onder druk en morele spanning | Kruispunten |
| Deel 3 | Bestemming | Terug- en vooruitkijken | Patroon zien en kompasrichting kiezen | Aankomst + horizon |

## Opdrachten per deel

| Deel | Opdracht | Status | Waar past dit het mooist? |
|---|---|---|---|
| Deel 1 | Privilegewiel (startpunt/zelfkennis) | Behouden | Begin |
| Deel 1 | Kernwaarden kiezen + niet-opgeefbare waarde | Behouden | Begin |
| Deel 1 | 4 realistische dilemma's (stap Onderweg binnen deel 1) | Behouden | Begin -> mini-onderweg |
| Deel 1 | STARR reflectie | Behouden | Einde Deel 1 |
| Deel 1 | Onverwachte SMS (Roosendaal) vlak voor PDF-print | Behouden | Afsluitende test Deel 1 |
| Deel 2 | Crossroads keuzedilemma met 4 afslagen | Behouden | Start Onderweg |
| Deel 2 | Waarde onder druk + handelen vs achteraf | Behouden | Midden Onderweg |
| Deel 2 | Vreemde ander + kompas-bijstelling | Behouden | Einde Onderweg |
| Deel 3 | Terugblik: scharnierpunt, patroon, ware noorden | Behouden (uit brainstorm) | Terugkijkend op bestemming |
| Deel 3 | Vooruitblik: nalatenschap, richting, belofte | Behouden (uit brainstorm) | Vooruitkijkend vanaf bestemming |
| Deel 3 | Synthese: Reisverslag 3.0 / persoonlijk kompasplan | Behouden (uit brainstorm) | Slot van trilogie |

## Aanbevolen flow in 1 regel

Deel 1 (Begin): positie + waarden + oefendilemma's + STARR + onverwachte SMS naar Deel 2 (Onderweg): Crossroads + morele spanning + bijstelling naar Deel 3 (Bestemming): terugkijken, patroon zien, vooruit kiezen.

## Signature events + afbeeldingen

Gebruik deze drie afbeeldingen in `public/images` met exact deze bestandsnamen:

- `public/images/sms-koud-station.jpg` -> Deel 1 signature: onverwachte SMS
- `public/images/crossroads-omleiding.jpg` -> Deel 2 signature: wegomleiding met 4 afslagen
- `public/images/brug-in-de-mist.jpg` -> Deel 3 signature: de brug in de mist

Smartphone mockups:

- `public/moral-maps-phone=art.jpg` -> Deel 1 smartphone mockup
- `public/moral-maps-phone-crossroads.jpg` -> Deel 2 smartphone mockup
- `public/images/mockup-deel3-bridge.jpg` -> Deel 3 smartphone mockup (gereserveerde naam zodra beschikbaar)

### Plek in de flow

- Deel 1 (sms): bij de afsluitende SMS-opdracht vlak voor de eindexport.
- Deel 2 (crossroads): bovenaan de dilemma/kruispunt-sectie als visuele trigger.
- Deel 3 (brug): in het reisverslag bij de signature-opdracht met ballast/essentieel/hoop.

## Terugvindplek stappen (vervolg)

Als je later verder wilt gaan, gebruik dit als startpunt:

- Hoofddocument met plan en keuzes:
  - `C:\Users\876409\moral aps\MAPS_TRILOGIE_OVERZICHT.md`
- Hoofdcode van de app:
  - `C:\Users\876409\moral aps\src\MoralMaps.jsx`
- Afbeeldingen:
  - `C:\Users\876409\moral aps\public\`

### Wat al klaarstaat

- Deel 1 flow + signature SMS.
- Deel 2 eerste flow (Crossroads + De Vreemde Ander) + eigen PDF/print.
- Deel 3 signature Brug in de Mist + portfolio PDF/print.
- Hervatten met participant code (basis aanwezig).

### Volgende stap na pauze

1. Preview openen op branch `feat/maps-flow-update`.
2. Volledige testrun: Deel 1 -> Deel 2 -> Deel 3.
3. Eventuele laatste tekst/UX-fixes.
4. Merge naar `main` wanneer preview akkoord is.

